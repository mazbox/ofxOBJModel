...
void ofxOBJGroup::calculateFlatNormals() {
	for(int i =0; i < faces.size(); i++) {
		faces[i].calculateFlatNormals();
	}
}
...
void ofxOBJGroup::calculateSmoothNormals() {

	calculateFlatNormals();
	
	// compare each point on each face with each other point and face
	for(int i = 0; i < faces.size(); i++) {
        printf("%d / %d\n", i, faces.size());
		for(int p = 0; p < faces[i].vertices.size(); p++) {
			glm::vec3 normal = faces[i].normal;
			int normalCount = 1;
			vector<glm::vec3> &verts = faces[i].vertices;
			
			for(int j = 0; j < faces.size(); j++) {
				if(i!=j) {
					if(faces[j].containsVertex(verts[p])) {
